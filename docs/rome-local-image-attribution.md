# Rome local image attribution

The Rome MVP stores a small preview image for each supported attraction inside the frontend public assets. These are locally served static files, not Google Places photo content. The image credits below must remain available in the product and repository.

罗马 MVP 为每个已支持景点在前端公共资源中保存一张本地预览图。这些图片以本地静态文件提供，不是 Google Places 照片内容。以下图片署名信息必须在产品和仓库中保留。
