...
- A Spring Boot modular monolith organised into the conventional `controller`, `service`, `repository`, and `entity` layers
...
| Java | 21 |
| Spring Boot | 4.0.7 |
| Spring Data JPA | Managed by Spring Boot 4.0.7 |
...
- Create a saved trip: `POST http://localhost:8080/api/v1/trips`
- Read a saved trip: `GET http://localhost:8080/api/v1/trips/{tripId}`
- Replace a saved trip: `PUT http://localhost:8080/api/v1/trips/{tripId}`
...
When Docker is available, Testcontainers checks MySQL, both Flyway migrations, JPA schema validation, Actuator, and the Trip API persistence flow. Without Docker, the integration test class is skipped while the unit and contract tests still run.
