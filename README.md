...
The recommended local command loads the root `.env`, starts MySQL, then starts Spring Boot:
...
中文说明：推荐直接运行 `./scripts/run-local-backend.sh`。它会自动加载根目录 `.env`、启动 MySQL，再启动 Spring Boot。如果 Docker Desktop 没有打开，脚本会给出明确提示，避免因为忘记加载 `.env` 导致 Google Places 未配置。
...
When Docker is available, Testcontainers checks MySQL, Flyway, and Actuator. Without Docker, the integration test class is skipped, while compilation and the Spring Modulith boundary test still run.
